...
The following visualizations are done to know the Summary of some of the variables at a glance which are known as univariate analysis.
```{r echo=FALSE, warning=FALSE, message=FALSE}
library(hrbrthemes)
h1 <- le %>%
...
h1
```
The above distribution, it is clear that the modal Life expectancy count falls between 72 to 78 ages.Very few people exists with Life expectancy of age around 90.
```{r echo=FALSE, warning=FALSE, message=FALSE}
library(hrbrthemes)
h2 <- le %>%
...
h2
```
Here, the summary of Adult Mortality Rates of both sexes (probability of dying between 15 and 60 years per 1000 population) is representing that the highest number lies around the number 25.This means the modal value is around 25 for Adult mortality.
```{r echo=FALSE, warning=FALSE, message=FALSE}
library(hrbrthemes)
h3 <- le %>%
...
h3
```
Hepatitis B (HepB) implies the immunization coverage among 1-year-olds (%) in the dataset. In the graph it is observed that the highest values of coverage is around 100%. 
## GDP at a glance:
```{r echo=FALSE, warning=FALSE, message=FALSE}
le %>%
  ggplot( aes(x=GDP)) +
    ...
    theme_ipsum()
```
The distribution of GDP, Gross Domestic Product per capita (in USD), shows that most of the countries per caripa GDP is below 12,000 USD.
...
```
The boxplot of total expenditure, General government expenditure on health as a percentage of total government,  expenditure (%)
...
ggplot(df2, mapping=aes(x=df2$Year, y=df2$`Life expectancy`)) +
    ...
    labs(tittle = "Year vs Life Expectancy",
     ...
     y = "Life expectancy")
```
To know whether the Life Expectancy is changing by Year or not I have chosen one Country(Canada). Here, the above bar diagram shows that in the Year 2006 to 2008 it has changed and then resulted the same ages of life expectancy. 

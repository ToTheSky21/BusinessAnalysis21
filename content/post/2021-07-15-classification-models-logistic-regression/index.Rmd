...
options(scipen=999)  
set.seed(100)
trainDataIndex <- createDataPartition(bank$y, p=0.7, list = F)  # 70% training data
trainData <- bank[trainDataIndex, ]
testData <- bank[-trainDataIndex, ]
head(testData)
```
...
```{r setup, include=TRUE}
logitmod1 <- glm(Class ~ age + job + default + loan , family = "binomial", data=down_train)
summary(logitmod1)
```
## Model with all dependent variable
```{r setup, include=TRUE}
logitmod2 <- glm(Class ~ . , family = "binomial", data=down_train)
summary(logitmod2)
```
## Model with contact, duration and poutcome
```{r setup, include=TRUE}
logitmod3 <- glm(Class ~ contact + duration + poutcome , family = "binomial", data=down_train)
summary(logitmod3)
```
The logit model is now built.Comparing above models in terms of null and residual deviance, we find for that the logitmodel3 gives the best result. As, we know,the smaller the AIC, the better the model fits the data.Also, in comparison of null and residual deviance, the above model gives best result.
